...
[ -z "$MC_LABEL" ] && MC_LABEL=$(run_script tw-config.sh labels.mission "" 2>/dev/null)
if [ -n "$MC_LABEL" ]; then
...
[ -z "$STATUS_PREFIX" ] && STATUS_PREFIX=$(run_script tw-config.sh labels.status_prefix "" 2>/dev/null)
