...
function AttributeRow({ label, value }) {
  return (
    <div className="flex justify-between items-baseline gap-3 py-1.5">
      <span className="text-white/40 shrink-0" style={{ fontSize: 'var(--text-xs)' }}>
        {label}
      </span>
      <span className="text-white/80 text-right" style={{ fontSize: 'var(--text-sm)' }}>
        {String(value)}
      </span>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span
              className="w-5 h-5 rounded-full bg-white/[0.08] flex items-center justify-center shrink-0 text-white/40"
              style={{ fontSize: 'var(--text-xs)' }}
            >
              {index + 1}
            </span>
            <h4 className="text-white/90 font-medium truncate" style={{ fontSize: 'var(--text-sm)' }}>
              {name}
            </h4>
          </div>
          {verdict && (
      ...
      {(pros?.length > 0 || cons?.length > 0) && (
        <div className="mt-3 border-t border-white/[0.05] pt-2 space-y-1.5">
          {pros?.map((p, i) => (
      ...
      {Object.keys(mergedAttrs).length > 0 && (
        <div className={`${pros?.length || cons?.length ? 'mt-2' : 'mt-3'} border-t border-white/[0.05] pt-2`}>
